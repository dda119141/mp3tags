...
static bool deleteFiles(const fs::path &filePath)
{
  ...
  for (auto DirElt = fs::directory_iterator(filePath.string());
       DirElt != fs::directory_iterator(); ++DirElt) {
    auto &dirElt = *DirElt;
...
void removeObsoleteDirectory(const fs::path &filePath,
                             const struct DirectoryOptions dirOption)
{
  auto funcObject = [&]() {
    ...
    for (auto &elt : fs::recursive_directory_iterator(filePath)) {
      if (findDirectoryToDelete(elt.path())) {
        std::cout << "Path to Delete: " << elt.path().string().c_str()
                  << std::endl;
    ...
    if (dirOption.remove)
      std::for_each(lists.begin(), lists.end(),
                    [](const fs::path &elt) { deleteFiles(elt); });
...
int main(int argc, const char **argv)
{
  ...
  const auto parser =
      ...
      lyra::opt(directory, "dir")["--directory"]["-d"](
          ...
          .required() |
      lyra::opt(DirectoryOption.empty)["--empty"]["-t"](
          "Get the empty frame content.") |
      lyra::opt(DirectoryOption.remove)["--remove"]["-a"](
          "Get the remove frame content.");
