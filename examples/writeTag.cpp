...
int main(int argc, const char **argv) {
  ...
  auto parser =
      ...
      lyra::opt(
          [&](std::string title) {
            gTags.push_back({std::string("title"), title});
          },
          "title")["--title"]["-t"]("Change the title frame content.") |
      lyra::opt(
          [&](std::string genre) {
            gTags.push_back({std::string("genre"), genre});
          },
          "genre")["--genre"]["-g"]("Change the genre frame content.") |
      lyra::opt(
          [&](std::string artist) {
            gTags.push_back({std::string("artist"), artist});
          },
          "artist")["--artist"]["-a"]("Change the artist frame content.") |
      lyra::opt(
          [&](std::string album) {
            gTags.push_back({std::string("album"), album});
          },
