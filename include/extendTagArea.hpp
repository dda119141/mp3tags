...
namespace id3v2
{
	...
	public:
		...
		explicit bufferExtended(const basicParameters& fileParameter, uint32_t additionalSize)
		{
			...
			tagBuffer = CreateTagBufferFromFile(fileParameter.get_filename(), GetTagHeaderSize<uint32_t>());
			...
			const auto tagSizeBuffer =
				updateTagSize(tagBuffer, additionalSize);
	...
	public:
			...
			:fileParameter(FileParameter)
		{
			std::call_once(m_once, [&content, additionalSize, this] {
				assert(fileParameter.get_frame_settings().getFramePayloadLength() >= content.size());

				const auto Buffer_obj = bufferExtended{ fileParameter, additionalSize };

				const auto filled_buffer = fillTagBufferWithPayload<std::string_view>(content, Buffer_obj.get_tag_buffer(),
	...
	private:
		...
		void ReWriteFile(id3::buffer_t tagBuffer, uint32_t extraSize) {
			...
			std::for_each(
				std::begin(*tagBuffer), std::end(*tagBuffer),
				[&filWrite](const char& n) { filWrite << n; });

			/* now write real audio payload*/
			std::for_each(
				bufRead->begin(), bufRead->begin() + fileSize,
				[&filWrite](const char& n) { filWrite << n; });
