...
namespace id3
{
	...
	public:
		...
		explicit id3_error(const std::string& msg)
			: audio_tag_error(msg.c_str())
		{
		}
	};
	...
	public:
		...
		explicit ape_error(const std::string& msg)
			: audio_tag_error(msg.c_str())
		{
		}
	};
#define ID3V2_THROW(msg) \
 ID3_THROW(id3_error, std::string("id3v2") + msg);

#define APE_THROW(msg) \
 ID3_THROW(ape_error, std::string("ape") + msg);
