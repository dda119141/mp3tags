...
namespace id3v2
{
    ...
    expected::Result<bool> UpdateFrameSize(UCharVec& buffer, uint32_t extraSize,
        ...
        id3v2::log()->info(" Tag Index: {}", tagIndex);

        id3v2::log()->info("Tag Frame Bytes before update : {}",
                           spdlog::to_hex(std::begin(buffer) + tagIndex,
                                          std::begin(buffer) + tagIndex + 4));

        /* reverse order of elements */
        std::reverse(it, it + NumberOfElements);

        std::transform(it, it + NumberOfElements, it, it, [&](uint32_t a,
                                                              uint32_t b) {
        ...
        id3v2::log()->info("Tag Frame Bytes after update : {}",
                           spdlog::to_hex(std::begin(buffer) + tagIndex,
                                          std::begin(buffer) + tagIndex + 4));
    ...
    expected::Result<bool> updateFrameSizeIndex(const iD3Variant& TagVersion,
                                                Args... args) {
        return std::visit(
            overloaded{
    ...
    const auto getTagPosFromEncodeByte(const cUchar& buff, uint32_t tagOffset,
        ...
        switch (buff[tagContentOffset]) {
            ...
            case 0x3: { /* UTF-8 [UTF-8] encoded Unicode [UNICODE]. Terminated
                           with $00 */
                const T tagLoc{tagOffset, tagContentOffset + 3, FrameSize - 3,
                               0x03};
                return expected::makeValue<TagInfos>(tagLoc);
                break;
            }
    ...
    template <typename T>
    std::optional<cUchar> prepareTagToWrite(T content, const TagInfos& tagLoc,
                                            cUchar& buffer) {
    ...
    std::optional<cUchar> prepareBufferWithNewTagContent(
        cUchar& buff, const std::string& content, const TagInfos& tagLoc) {
        assert(tagLoc.getLength() >= content.size());
    ...
    struct newTagArea {
    public:
        explicit newTagArea(const std::string& filename, const TagInfos& tagLoc,
                            const iD3Variant& tagVersion,
            ...
            std::call_once(m_once, [&filename, &tagLoc, &tagVersion,
                ...
                if (ret.has_value()) {
                    ...
                    auto ret1 =
                        extendBuffer(tagLoc, additionalSize, tagVersion) |
                        [&](const cUchar& buffer) {
                            mCBuffer = buffer;
    ...
    private:
        ...
        expected::Result<T> constructNewTagInfos(const T& tagLoc,
                                                 uint32_t newtagSize) {
            const T TagLoc{tagLoc.getTagOffset(), tagLoc.getTagContentOffset(),
                           tagLoc.getLength() + newtagSize,
                           tagLoc.getEncodingValue(), tagLoc.getSwapValue()};

            return expected::makeValue<T>(TagLoc);
        }

        expected::Result<cUchar> extendBuffer(const TagInfos& tagLoc,
                                              uint32_t additionalSize,
                                              const iD3Variant& tagVersion) {
            const auto ret = mCBuffer | [&](cUchar& cBuffer) {
                ...
                updateFrameSizeIndex<cUchar&, uint32_t, uint32_t>(
                    tagVersion, cBuffer, additionalSize, tagLoc.getTagOffset());
                ...
                ID3_LOG_INFO("FrameSize... length: {}, tag start: {}",
                             tagLoc.getLength(), tagLoc.getTagOffset());

                ID3_LOG_INFO(
                    "Tag Frame Bytes after update : {}",
                    spdlog::to_hex(
                        std::begin(cBuffer) + tagLoc.getTagOffset() + 4,
                        std::begin(cBuffer) + tagLoc.getTagOffset() + 7));
    ...
    public:
        ...
        expected::Result<bool> WriteFile(const std::string& content,
            ...
            for (uint32_t i = 0; i < (tagLoc.getLength() - content.size());
                 ++i) {
                filWrite << '\0';
            }
        ...
        const expected::Result<TagInfos> findTagInfos(
...

            {
                ...
                //   id3v2::log()->info(" tag name: {}", tag);
                // std::string erg = std::string(tag);
                // std::string erg = "test";
                // id3v2::log()->info(" tag name:");
