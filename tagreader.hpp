...
auto GetHeader(const std::string& _filename )
{
...
template <typename T>
const auto GetTagArea(const std::vector<char>& buffer)
{
...
class retrieveTag
{
	...
	std::optional<const int> operator() (const type& tag)
	{
		auto header = id3v2::GetHeader(_filename);
		if(!header.has_value())
			return {};
		...
		if(it.has_value()){
			std::cout << "Retrieve tag : " << tag << " at position: " << it.value() << std::endl;
		}
